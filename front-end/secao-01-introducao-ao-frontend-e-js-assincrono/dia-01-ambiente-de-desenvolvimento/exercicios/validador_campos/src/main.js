...
button.addEventListener('click', () => {
  ...
  if (select.value === 'email') {
    res = validator.isEmail(inputText.value);
  } else if (select.value === 'cpf') {
    const inputJustNumbers = inputText.value.replace(/\./g, '');
    res = validator.isInt(inputJustNumbers)
    && validator.isLength(inputJustNumbers, { min: 11, max: 11 });
  }
  result.innerHTML = `A validação retornou ${res}`;
});

// Fiz somente com 3 opções. O debaixo é do prof.
